# altai-shell-integration (bashrc)
#
...
if [ -z "$__ALTAI_HOOKS_LOADED" ]; then
  ...
  _altai_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_altai_urlencode "$PWD")"
    if [ -z "$__ALTAI_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __ALTAI_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_altai_precmd:"*) ;;
    *) PROMPT_COMMAND="_altai_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
