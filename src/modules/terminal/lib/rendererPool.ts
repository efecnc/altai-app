...
function getRecycler(): HTMLDivElement {
  ...
  el.setAttribute("data-altai-recycler", "");
  el.style.cssText =
...
function createSlot(): Slot {
  ...
  host.setAttribute("data-altai-slot", String(slots.length));
...
function bindSlot(slot: Slot, p: AcquireParams): void {
  ...
  if (p.snapshot) {
    try {
      ...
      console.warn("[altai] snapshot replay failed:", e);
    }
  }
  if (p.altScreen) {
...
function serializeSlot(slot: Slot): SerializeOutput {
  ...
  try {
    ...
    console.warn("[altai] serialize failed:", e);
  }
  return {
    snapshot,
    cols: slot.term.cols,
...
function attachWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[altai-webgl] unavailable:", e);
  }
}
...
function disposeSlotWebgl(slot: Slot): void {
  ...
  try {
    ...
    console.warn("[altai-webgl] dispose failed:", e);
  }
  try {
    const r = (
