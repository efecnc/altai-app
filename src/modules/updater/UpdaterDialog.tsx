...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S altai-bin";
    case "debian":
      return `sudo apt install ./ALTAI_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./ALTAI-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `ALTAI v${manual.version} is available`
                  : `ALTAI v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart ALTAI to finish installing."
              : downloading
                ? progress !== null
