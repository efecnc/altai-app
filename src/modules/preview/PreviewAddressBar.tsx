...
type Props = {
  ...
  onReload: () => void;
  // Opens the current URL as a native child-webview tab — the escape hatch
  // for sites (Colab, banks, anything with X-Frame-Options) that refuse to
  // render inside the iframe-based preview.
  onOpenAsWebview: (url: string) => void;
};

export const PreviewAddressBar = forwardRef<PreviewAddressBarHandle, Props>(
  function PreviewAddressBar({ url, onSubmit, onReload, onOpenAsWebview }, ref) {
    ...
    const submit = () => {
      ...
      setNotice(null);
      // Sites that refuse iframe embedding can crash the preview WebView on
      // some platforms (Colab on macOS WKWebView, observed). Route them to a
      // native webview tab so the broken iframe path is never taken.
      if (looksLikeIframeBlocker(next)) {
        onOpenAsWebview(next);
        return;
      }
    ...
    return (
      ...
      <div className="flex h-9 items-center gap-1 bg-card/40 px-1.5">
        ...
        <Button
          ...
          size="icon"
          onClick={() => {
            const next = normalizeUrl(draft) ?? url;
...
// Hosts known to either refuse iframe embedding (X-Frame-Options / CSP
// frame-ancestors) or crash the embedding WebView outright when attempted.
// Keep this list tight — the cost of a false positive is silently bypassing
// the lightweight iframe preview for a site that would have worked.
function looksLikeIframeBlocker(url: string): boolean {
  try {
    const host = new URL(url).hostname.toLowerCase();
    return (
      host === "colab.research.google.com" ||
      host === "colab.google.com" ||
      host.endsWith(".colab.google.com") ||
