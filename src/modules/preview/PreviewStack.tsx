...
type Props = {
  ...
  onOpenAsWebview: (id: number, url: string) => void;
...
export function PreviewStack({
  ...
  const openWebviewRef = useRef(onOpenAsWebview);
  useEffect(() => {
  ...
  useEffect(() => {
    openWebviewRef.current = onOpenAsWebview;
  }, [onOpenAsWebview]);
  ...
  const webviewCallbacks = useRef(
    new Map<number, (url: string) => void>(),
  );
  ...
  const getOpenAsWebviewCallback = (id: number) => {
    let cb = webviewCallbacks.current.get(id);
    if (!cb) {
      cb = (url: string) => openWebviewRef.current(id, url);
      webviewCallbacks.current.set(id, cb);
    }
    return cb;
  };
  ...
  useEffect(() => {
    ...
    for (const id of webviewCallbacks.current.keys()) {
      if (!live.has(id)) webviewCallbacks.current.delete(id);
    }
  }, [previews]);
        ...
        return (
          ...
          >
            <PreviewPane
              ...
              onOpenAsWebview={getOpenAsWebviewCallback(t.id)}
            />
          </div>
        );
      })}
