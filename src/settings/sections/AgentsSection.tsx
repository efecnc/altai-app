...
import { type Agent, type AgentIconId } from "@/modules/ai/lib/agents";
import {
  isValidHandle,
  normalizeHandle,
  type Snippet,
...
export function AgentsSection() {
  ...
  // Subscribe to each underlying slice so the card grid re-renders on any
  // change (custom agents, disabled set, builtin overrides). The data is
  // then read through `all()` for convenience.
  ...
  const setAgentDisabled = useAgentsStore((s) => s.setDisabled);
  const resetBuiltinAgent = useAgentsStore((s) => s.resetBuiltin);
  const hydrateAgents = useAgentsStore((s) => s.hydrate);

  // `customAgents` is read indirectly through `all()` below; this discard
  // keeps the Zustand subscription above active so additions/removals of
  // custom agents re-render the card grid.
  void customAgents;
        ...
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
          {allAgents.map((a) => (
            <AgentCard
              ...
              overridden={Object.prototype.hasOwnProperty.call(overrides, a.id)}
              onToggleEnabled={(enabled) => setAgentDisabled(a.id, !enabled)}
              onEdit={() => setEditingAgent(a)}
              onResetDefaults={
                a.builtIn ? () => resetBuiltinAgent(a.id) : null
              }
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <span className="flex items-center gap-1.5 text-[12.5px] font-medium">
            ...
            {overridden ? (
              <span
                className="rounded bg-foreground/10 px-1 py-0.5 text-[9px] tracking-wide text-foreground/70 uppercase"
                title="You have edited this built-in agent."
              >
                Edited
              </span>
            ) : null}
        ...
        <Switch
          ...
          title={
            disabled
              ? "Enable this agent (show it in the picker)"
              : "Disable this agent (hide it from the picker)"
          }
        />
      </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pr-1">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Textarea
              ...
              placeholder="Persona & rules. Appended to ALTAI's core system prompt."
